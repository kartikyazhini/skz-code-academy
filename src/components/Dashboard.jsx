...
const Dashboard = ({ modules = [], userProgress, onStart }) => {
  const handleHardReset = () => {
    if (window.confirm("ARE YOU SURE? This will delete all training progress. You cannot undo this!")) {
      localStorage.clear();
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <div className="bg-zinc-900/20 border border-zinc-800/50 p-8 rounded-[2.5rem] flex items-center gap-5 backdrop-blur-sm">
            ...
            <div>
              <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Active Streak</p>
          ...
          <div className="bg-zinc-900/20 border border-zinc-800/50 p-8 rounded-[2.5rem] flex items-center gap-5 backdrop-blur-sm">
            ...
            <div>
              <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Sections Completed</p>
              <p className="text-xl font-black italic uppercase">{(userProgress?.currentSection || 1) - 1}</p>
            </div>
          </div>
          <div className="bg-zinc-900/20 border border-zinc-800/50 p-8 rounded-[2.5rem] flex items-center gap-5 backdrop-blur-sm">
            <Shield className="text-blue-500" size={24} />
            <div>
              <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Rank</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {modules.map((module) => {
            ...
            // Logic to check module status
            const firstSectionId = module.sections[0].id;
            const lastSectionId = module.sections[module.sections.length - 1].id;

            const isLocked = firstSectionId > currentSectionId;
            const isCompleted = lastSectionId < currentSectionId;
            const isActive = currentSectionId >= firstSectionId && currentSectionId <= lastSectionId;

            // Find the specific section user is on WITHIN this module
            const activeSectionInModule = module.sections.find(s => s.id === currentSectionId) || module.sections[0];

            return (
              <motion.div
                key={module.id}
                whileHover={!isLocked ? { y: -5 } : {}}
                className={`relative bg-zinc-900/10 border border-zinc-800/50 rounded-[3.5rem] p-10 shadow-2xl backdrop-blur-md transition-all group overflow-hidden ${isLocked ? 'opacity-40 grayscale' : 'hover:border-skz-green/30'}`}
              >
                ...
                <div className="relative z-10 flex flex-col h-full justify-between min-h-[200px]">
                  ...
                  <div className="mt-8 flex items-center justify-between">
                    {isLocked ? (
                      <div className="flex items-center gap-2 text-zinc-600 font-black text-[10px] uppercase tracking-widest">
                        <Lock size={14} /> Protocol Locked
                      </div>
                    ) : isCompleted ? (
                      <div className="flex items-center justify-between w-full">
                        <span className="text-skz-green font-black uppercase text-[10px] flex items-center gap-2">
                          <CheckCircle size={16}/> Mastered
                        </span>
                        <button
                          onClick={() => onStart(module.id, module.sections[0].id)}
                          className="text-zinc-500 hover:text-white flex items-center gap-2 text-[10px] font-black uppercase"
                        >
                      ...
                      <>
                        <div className="flex flex-col">
                          ...
                          <span className="text-skz-green font-black uppercase italic text-sm">{activeSectionInModule.title}</span>
                        </div>
                        <button
                          onClick={() => onStart(module.id, activeSectionInModule.id)}
        ...
        <div className="mt-20 flex justify-center">
          <button
            ...
            className="flex items-center gap-2 text-zinc-800 hover:text-red-500 transition-colors text-[10px] font-black uppercase tracking-[0.3em]"
          >
